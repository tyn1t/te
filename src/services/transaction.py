...
class TransactionService:
    async def read_all(self, account_id: int, limit: int, skip: int = 0) -> list[Record]:
        query = transactions.select().where(transactions.c.accounts_id == account_id).limit(limit).offset(skip)
        return await database.fetch_all(query)

    @database.transaction()
    async def create(self, transaction: TransactionIn) -> Record:
        query = accounts.select().where(accounts.c.id == transaction.accounts_id)
        ...
        await self.__update_account_balance(transaction.accounts_id, balance)
    ...
    async def __register_transaction(self, transaction: TransactionIn) -> int:
        command = transactions.insert().values(
            accounts_id=transaction.accounts_id,
